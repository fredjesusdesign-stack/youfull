  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div>
        ...
        <input name="title" type="text" required defaultValue={post?.title} className={inputClass} placeholder="Título do artigo" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input name="tags" type="text" defaultValue={post?.tags?.join(', ') ?? ''} className={inputClass} placeholder="yoga, bem-estar" />
        </div>
      </div>
      ...
      <div>
        ...
        {thumbnailUrl && <button type="button" onClick={() => setThumbnailUrl('')} className="text-xs text-red-500 mt-1 hover:underline">Remover</button>}
      </div>

      {/* Video URL */}
      <div>
        <label className={labelClass}>URL do vídeo (opcional)</label>
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" className="px-6 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-full text-sm font-medium transition-colors">
          {post ? 'Guardar alterações' : 'Criar post'}
        </button>
        <a href="/admin/blog" className="px-6 py-2.5 border border-border text-text-muted hover:text-text rounded-full text-sm transition-colors">Cancelar</a>
      </div>
    </form>
  )
}
